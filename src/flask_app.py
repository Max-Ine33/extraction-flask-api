...
def get_articles():
    ...
    articles = arxiv_crawler.get_arxiv_articles(
        query=search, start=start, max_results=max_results
    )
...
@app.route(
    "/articles/<path:id>", methods=["GET"]
)  # Used <path:> because there is a / in the id of an article
